...
<body>
	...
	<main role="main">
		<div class="container">
			<form action="/jejuOseyo/Admin/HostList.do" method="post">
							...
							<tbody>
								<c:forEach items="${hostList }" var="host">
									...
									<tr>
										<td>${host.rep }</td>
										<td><a
											href="/jejuOseyo/Admin/AdHostView.do?hid=${host.hid }&pageNum=${pageNum}&type=${type }&keyword=${keyword}">
												${host.hid }</a></td>
										<td>${host.hphone }</td>
										<td>${host.hemail }</td>
										<td>${host.crnum }</td>
						...
						<ul class="pagination justify-content-center">
							...
							<c:if test="${prev }">
								<li class="page-item"><a
									href="/jejuOseyo/Admin/HostList.do?pageNum=${start - 1 }&type=${type }&keyword=${keyword }"
							...
							<c:forEach begin="${start }" end="${end }" var="i">
								<c:url var="link"
									value="/Admin/HostList.do?pageNum=${i }&type=${type }&keyword=${keyword }" />
							...
							<c:if test="${next }">
								<li class="page-item"><a
									href="/jejuOseyo/Admin/HostList.do?pageNum=${end + 1 }&type=${type }&keyword=${keyword }"
