			...
			<div class="col-4 text-right"> 
				<span class="btn btn-secondary">
					전체 게시물 ${totalCnt }건</span>
					 <a class="btn btn-primary" href="/jejuOseyo/Room/SortByStar.do?pageNum=${1}&keyword=${keyword }&people=${people }&checkIn=${param.checkIn }&checkOut=${param.checkOut }">별점순 정렬</a></div></div>
...
<c:if test="${not empty searchRoomList}">
    ...
    <%-- 페이징 --%>
			<ul class="pagination justify-content-center">
				...
				<c:if test="${prev }">
				<li class="page-item">
					<a href="/Room/SearchRoomList.do?pageNum=${start - 1 }&keyword=${keyword }&people=${people }&checkIn=${checkIn }&checkOut=${checkOut }" 
					   class="page-link">&laquo; Previous</a></c:if>
				
				<%-- 페이지 번호 버튼 --%>
				<c:forEach begin="${start }" end="${end }" var="i">
				<c:url var="link" value="/Room/SearchRoomList.do?pageNum=${i}&keyword=${keyword }&people=${people }&checkIn=${checkIn }&checkOut=${checkOut }"/>
				...
				<c:if test="${next }">
				<li class="page-item">
					<a href="/Room/SearchRoomList.do?pageNum=${end+1 }&keyword=${keyword }&people=${people }&checkIn=${checkIn }&checkOut=${checkOut }"  
