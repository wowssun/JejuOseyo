...
<head>
<meta charset="UTF-8">
<title>yeyakList.jsp</title>
</head>
<body>

<%@ include file="../include/header.jsp" %>

<!-- 예약 완료/취소 메시지가 있는 경우 알림창으로 출력 -->
<c:if test="${!empty msg }">
	<script>
		alert('${msg}');
	</script>
...
<div class="jumbotron" style="background-color:#FFFBC9" >
	...
	<main role="main">
		<div class="container">
		<form action="/jejuOseyo/Yeyak/yeyakList.do" method="post">	
		<!-- 예약목록이 비어 있는 경우-->
		<c:if test="${empty yeyakList }">
			<div class="row">
	...
	<c:forEach items="${yeyakList }" var="yvo">
		<c:url var="yeviewlink" value="/Yeyak/yeyakView.do">
 		...
 		<div class="text-right">${yeyakDt }</div>
		 	<div class="row">
				<div class="col-md-4">
					...
					<div class="col-md-8">
						<h4 style="margin-top: 50px"><a href="${yeviewlink }" >${yvo.rvo.rmName }</a></h4>
			...
			<ul class="pagination justify-content-center mt-5">
				...
				<c:if test="${prev }">
				<li class="page-item">
					<a href="/Yeyak/yeyakList.do?pageNum=${start - 1 }" 
					   class="page-link">&laquo; Previous</a></c:if>
				
				<%--페이지 번호 버튼 --%>
				
				<c:forEach begin="${start }" end="${end }" var="i">
				<c:url var="link" value="/Yeyak/yeyakList.do?pageNum=${i }">			
				</c:url>
				...
				<c:if test="${next }">
				<li class="page-item">
					<a href="/Yeyak/yeyakList.do?pageNum=${end + 1 }" 
